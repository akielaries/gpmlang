...
headers : headers headers | INCLUDE {
    add('H');
};
...
'(' statement ';' condition ';' statement ')' '{' body '}' | IF {
    add('K');
}
'(' condition ')' '{' body '}' else | statement ';' | body body | PRINTFF {
    add('K');
}
'(' STR ')' ';' | SCANFF {
    add('K');
...
condition : value relop value | TRUE {
    add('K');
}
| FALSE {
    add('K');
...
init | ID '=' expression | ID relop expression | ID UNARY | UNARY ID;

init : '=' value | ;

expression : expression arithmetic expression | value;
